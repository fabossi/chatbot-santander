...
export class ChatInputComponent implements OnInit, OnDestroy {
  ...
  onSubmitText() {
    ...
    const userInfo: User = { text: this.inputForm.get('inputText').value, type: 'user', time: now, tempMessage: '' };

    if (this.inputForm.get('inputText').value !== ('' || undefined) && !this.inputForm.get('inputText').errors) {
      this.chatService.sendResponseFromWatson(this.inputForm.get('inputText').value);
      ...
      const userMessageError: User = {
        text: 'Refaça sua frase com pelo menos 3 caracteres.', type: 'watson', time: null, tempMessage: 'Campo obrigatório'
      };
  ...
  createForm() {
    this.inputForm = new FormGroup({ inputText: new FormControl('', [Validators.required, Validators.minLength(3)]) });
  }

  showRecentMessages() {
    this.chatService.viewRecentMessage.next(this.showRecentMsg = !this.showRecentMsg);
  }
